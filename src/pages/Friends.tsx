          ...
          <div className="space-y-6">
            {friendUsers.map(user => (
              <div key={user.id} className="friend-card">
                ...
                <div className="flex-grow">
                  ...
                  <p className="text-gray-300 text-sm my-1 line-clamp-2">{user.about}</p>
                  
                  <div className="flex gap-6 mt-2 text-sm text-gray-400">
                    <div>
                      <span className="text-gameverse-red">{user.friends}</span> друзей
                    </div>
                    <div>
                      <span className="text-gameverse-red">{user.blogs}</span> блогов
                    </div>
                    <div>
                      <span className="text-gameverse-red">{user.reviews}</span> рецензий
                    </div>
                  </div>
                </div>
            ...
            {friendUsers.length === 0 && (
              <div className="text-center py-8 bg-gameverse-light rounded-lg">
                <p className="text-gray-400">У вас пока нет друзей. Добавьте их из списка пользователей.</p>
              </div>
            )}
          </div>
        )}
